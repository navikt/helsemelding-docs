...
```
  Helsemelding                      Kafka                         Fagsystem
      │                               │                               │
      │──── publish message ────▶ [in.json] ────── consume ──────────▶│
      │                                                               │
      │                        Attachment Service                     │
...
1. Consuming inbound messages from `helsemelding.dialog.in.json`
...
## Step 1: Consume a message from `helsemelding.dialog.in.json`

Inbound dialog messages are published to `helsemelding.dialog.in.json` after they have been
received from the external healthcare provider, validated, and converted from XML to JSON by the
Helsemelding platform.

### Record key

The Kafka record key is a **UUID** that uniquely identifies the message. Use this ID to fetch
attachments from the Attachment Service.
...
```json
{
  ...
  "conversationReference": {
    "parentMessageId": "a8b2d381-5ba1-48d4-91c4-790bc1dde4c4",
    "conversationId": "65f15ff2-b3fa-4b45-ab92-8906c13d9402"
  },
...
using the Kafka record key as the `messageId`.
...
Contact Team Helsemelding, so that we can add innbound rule in `helsemelding-attachment-service` for your application.
...
By the time a message is available on `in.json`, its attachments are already stored in the
